...
export default function About() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0e1a]">
        ...
        <div className="relative z-10 container mx-auto px-6 pt-36 pb-24 max-w-6xl">
          <RevealSection variant="up">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              <button onClick={() => navigate('/strategies')} className="inline-flex items-center gap-2 px-6 py-3 bg-emerald-500 hover:bg-emerald-400 text-white font-bold text-sm transition-all">View Strategies <ArrowRight className="w-4 h-4" /></button>
              <button onClick={() => navigate('/community')} className="inline-flex items-center gap-2 px-6 py-3 border border-white/[0.20] text-white/70 hover:text-white hover:border-white/40 font-semibold text-sm transition-all">Join Community</button>
            </div>
          </RevealSection>
        </div>
      </section>
          ...
          <div className="container mx-auto px-6 max-w-6xl">
            <div className="flex flex-col sm:flex-row items-stretch divide-y sm:divide-y-0 sm:divide-x divide-white/[0.07]">
              {stats.map(({ value, suffix, label }) => (
                <div key={label} className="flex-1 py-8 px-10 flex items-center gap-5">
                  <p className="text-4xl font-black text-white tabular-nums">{value}<span className="text-emerald-400">{suffix}</span></p>
        ...
        <div className="container mx-auto px-6 max-w-6xl">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <RevealSection variant="right">
              ...
              <div className="flex items-center gap-0 border border-white/[0.08] bg-[#0f1420]">
                {[{ v: '67%', l: 'Avg Win Rate' }, { v: '2yrs', l: 'Track Record' }, { v: '12+', l: 'Strategies' }].map(({ v, l }, i) => (
                  <div key={l} className={`flex-1 px-6 py-5 ${i < 2 ? 'border-r border-white/[0.08]' : ''}`}>
          ...
          <RevealSection variant="stagger">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-white/[0.05]">
              {values.map(({ icon: Icon, title, body }) => (
                <div key={title} className="bg-[#060a14] p-7 border-l-2 border-emerald-500 hover:bg-[#0f1420] transition-colors group cursor-default">
                ...
                <RevealSection key={item.title} variant="left">
                  <div className="flex items-start gap-8 pb-10">
                    <div className="relative z-10 shrink-0 mt-1">
                      <div className="w-9 h-9 bg-[#9966ff]/20 border border-[#9966ff]/30 flex items-center justify-center">
                        <span className="text-[#9966ff] text-[10px] font-black">{item.year.slice(2)}</span>
                      </div>
                    </div>
                    <div className="flex-1 bg-[#0f1420] border border-white/[0.07] p-6 hover:border-white/[0.12] transition-colors">
          ...
          <RevealSection variant="stagger">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {team.map(({ name, role, bio, img }) => (
                <div key={name} className="border border-white/[0.08] bg-[#0f1420] hover:border-white/[0.15] transition-colors group">
                  <div className="relative h-56 overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-[#0f1420] via-[#0f1420]/20 to-transparent" />
                  </div>
      ...
      {/* CTA */}
      <section className="relative py-28 overflow-hidden bg-[#060a14]">
        <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: 'linear-gradient(rgba(255,255,255,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.03) 1px, transparent 1px)', backgroundSize: '60px 60px' }} />
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-purple-500/[0.08] rounded-full blur-3xl pointer-events-none" />
        <div className="relative z-10 container mx-auto px-6 max-w-4xl text-center">
          <RevealSection variant="up">
            ...
            <div className="flex items-center justify-center gap-4 flex-wrap">
              <button onClick={() => navigate('/strategies')} className="inline-flex items-center gap-2 px-6 py-3 bg-emerald-500 hover:bg-emerald-400 text-white font-bold text-sm transition-all">Browse Strategies <ArrowRight className="w-4 h-4" /></button>
              <button onClick={() => navigate('/community')} className="inline-flex items-center gap-2 px-6 py-3 border border-white/20 text-white/70 hover:text-white hover:border-white/40 font-semibold text-sm transition-all">Join Community</button>
            </div>
          </RevealSection>
        </div>
      </section>
